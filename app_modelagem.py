    ...
    with tab_manual:
        ...
        with col1:
            ticker_sugerido = st.selectbox(
                "Ticker sugerido",
                options=["(digitar manualmente)"] + TICKERS_SUGERIDOS,
            )
            ticker_manual = st.text_input(
                "Ou digite o ticker:",
                value="" if ticker_sugerido == "(digitar manualmente)" else ticker_sugerido,
                placeholder="Ex: BBAS3.SA, AAPL, NVDA",
            )
            ticker_input = ticker_manual.strip().upper() if ticker_manual.strip() else ticker_sugerido
