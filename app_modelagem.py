...
<style>
    ...
    [data-testid="stSidebar"]          { background-color: #0D1627; border-right: 1px solid #1E3A4A; }
    ...
    div[data-testid="stMetricValue"]   { color: #2DD4BF !important; font-size: 1.6rem !important; }
...
def baixar_retornos(tickers: list, janela: int = 252) -> pd.DataFrame:
    ...
    if not frames:
        return pd.DataFrame()
    ret = pd.DataFrame(frames).dropna()
    return ret.tail(janela)
...
st.sidebar.markdown("**Modelagem Aplicada ao Mercado Financeiro**")
st.sidebar.divider()
...
status_carteira = "✅ carregada" if st.session_state["carteira"] is not None else "❌ não carregada"
status_var = "✅ calculado" if st.session_state["resultados"] else "❌ não calculado"
st.sidebar.markdown(
    f"**Status**\n\n"
    f"- Confiança: `{p['confianca']*100:.0f}%`\n"
    f"- Horizonte: `{p['horizonte']} dia(s)`\n"
    f"- Janela: `{p['janela']} dias`\n"
    f"- Simulações: `{p['n_sim']:,}`\n"
    f"- Carteira: `{status_carteira}`\n"
    f"- VaR: `{status_var}`"
)
st.sidebar.caption("Projeto Final — Modelagem Aplicada ao Mercado Financeiro")
...
# =============================================================================
elif pagina == "📈 Construção da Carteira":
...
# =============================================================================
    ...
    with col1:
        ...
        msgs = {
            0.90: "📌 90% — 1 excesso esperado a cada 10 dias.",
            0.95: "📌 95% — Padrão de mercado. 1 excesso a cada 20 dias.",
            0.99: "📌 99% — Regulatório (Basileia III). 1 excesso a cada 100 dias.",
        }
        st.info(msgs[confianca])
    ...
    with col2:
        ...
        janela = st.slider(
            "Número de dias úteis históricos:",
            min_value=60, max_value=504, step=21,
            value=st.session_state["params"]["janela"],
        )
        st.caption("252 dias = 1 ano útil. Janelas maiores capturam mais ciclos de mercado.")
        ...
        n_sim = st.select_slider(
            ...
            value=st.session_state["params"]["n_sim"],
    ...
    if st.button("💾 Salvar parâmetros", type="primary"):
        ...
        # Forçar recálculo dos retornos com nova janela
        st.session_state["retornos"]   = None
        st.session_state["resultados"] = {}
        st.success(f"✅ Parâmetros salvos: {confianca*100:.0f}% | {horizonte} dia(s) | {janela} dias | {n_sim:,} sim.")
...
if pagina == "📐 VaR Paramétrico":
    ...
    if not cov_matrix.empty:
        st.dataframe(
            cov_matrix.style.format("{:.6f}").background_gradient(cmap="YlOrRd"),
    ...
    if len(ret_carteira) > 0:
        ...
        with col1:
            ...
            fig_hist.add_trace(go.Histogram(
                ...
                marker_color="#2DD4BF", opacity=0.7, name="Retornos",
            ))
        ...
        if len(perdas) > 0:
            ...
            with col2:
                ...
                fig_cdf.add_trace(go.Scatter(
                    ...
                    mode="lines", line=dict(color="#2DD4BF", width=2), name="CDF",
                ))
                fig_cdf.add_hline(
    ...
    if res:
        ...
        with col2:
            fig_comp = go.Figure(go.Bar(
                ...
                marker_color=["#2DD4BF", "#818CF8", "#F59E0B"][:len(df_comp)],
