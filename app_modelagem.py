...
<style>
    ...
    .stTabs [aria-selected="true"]     { background-color: #2DD4BF !important; color: #0B1120 !important;
                                         font-weight: 700; }
...
with col_status:
    st.markdown(
        ...
        f"Confiança: <b style='color:#2DD4BF'>{p['confianca']*100:.0f}%</b> &nbsp;|&nbsp; "
        f"Horizonte: <b style='color:#2DD4BF'>{p['horizonte']}d</b> &nbsp;|&nbsp; "
        f"Janela: <b style='color:#2DD4BF'>{p['janela']}d</b> &nbsp;|&nbsp; "
        f"Carteira: <b style='color:#2DD4BF'>{status_carteira}</b> &nbsp;|&nbsp; "
        f"VaR: <b style='color:#2DD4BF'>{status_var}</b>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
with tab_param_var:
    ...
    if not garantir_retornos():
        st.warning("Configure a carteira e os parâmetros antes de calcular.")
